...
- **무엇을 미루나**: **자동 요약·핵심 도출은 v1에서 만들지 않는다.** 대신 붙일 자리(분석 슬롯 — 2.4절)만 만든다. 스크립트 추출을 완성해 배포한 뒤(P4) 붙인다(P5). 단, **수동 흐름은 v1 에 있다**(v2): 사용자가 스크립트를 버튼 한 번으로 복사해 자기 AI 챗 서비스에 붙이고, 받은 요약·정리를 '요약 및 정리' 메뉴에 붙여 넣어 저장한다.
...
화면(P3)은 스크립트 **전체 텍스트를 버튼 한 번으로 클립보드에 복사**한다(타임스탬프 포함/제외, 요약 요청 프롬프트 머리말 포함 여부는 P3 설계에서 정한다). 이것이 v1 의 요약 경로다 — 사용자가 자기 AI 챗 서비스에 붙여 넣는다(v2).

### 2.4 분석 슬롯 — 자동 분석은 자리만, 수동 요약·정리는 v1 (v2)

요약·핵심 도출·챕터 나누기 같은 **자동 분석(analysis)** 은 P5에서 붙인다. v1이 미리 만들어 두는 것:

| 요소 | v1 상태 |
|---|---|
| `Analyzer` 인터페이스(Protocol) | 정의 (P0) |
| 분석기 레지스트리 | `NullAnalyzer`만 등록 (P0) |
| `analyses` 테이블 | P1 에서 생성. **수동 입력 데이터는 P3 부터** 들어간다 |
| API `GET /analyzers`, `POST /videos/{id}/analyses` | 분석기 목록은 빈 값. `POST` 는 **수동(`manual`) 입력이면 저장(201)**, 자동 분석기 요청이면 `501 Not Implemented` + 사용 가능 분석기 목록 (P3) |
| 화면 '요약 및 정리' 탭 | **붙여넣기 편집기 + 저장**(수동 흐름). 자동 분석 버튼은 "설정된 분석기가 없습니다" 안내 (P3) |
...
**수동 흐름(v1, v2 개정)**: 사용자가 스크립트를 복사해 외부 AI 챗 서비스에서 요약·핵심 정리를 받고, 그 글을 '요약 및 정리' 메뉴에 붙여 넣어 저장한다. 저장은 자동 분석기와 **같은 `analyses` 테이블**에 `analyzer_name="manual"`, `analyzer_version="user"` 로 들어간다(`constants.MANUAL_ANALYZER_NAME`). 종류(`summary`·`keypoints`)별로 영상당 1건을 편집·재저장하고 저장 시각을 남긴다. 어느 서비스에서 받았는지는 선택 입력(`model_info.service`). P5 가 자동 분석을 붙여도 수동 글은 그대로 남는다 — 같은 화면에서 출처(`manual`/분석기 이름)로 구분한다. 수동 입력은 계산이 없으므로 `Analyzer` 레지스트리를 거치지 않는다.

### 2.5 사용자 화면

| 단계 | 형태 | 핵심 화면 |
|---|---|---|
| P3 | 로컬 웹앱 (브라우저) | 채널 추가·동기화 / 롱폼·숏폼 탭 그리드 / 스크립트 뷰어(타임스탬프 클릭 → 영상 위치, **원클릭 복사**) / **'요약 및 정리'(붙여넣기 편집·저장, 자동 분석 자리)** / 작업 모니터 / 설정 |
...
Python·ffmpeg·Node가 없는 **깨끗한 Windows PC**에서 설치파일 하나로 설치 → 채널 1개 추가 → 롱폼·숏폼 목록 전량 → 자막 있는 영상은 즉시, 없는 영상은 Whisper로 스크립트 열람 → **'복사' 버튼으로 전체 텍스트가 클립보드에 들어가고, '요약 및 정리'에 붙여 넣은 글이 저장·재표시된다** → 자동 분석 버튼은 "설정된 분석기 없음" 안내가 보인다.
...
| 3 | Vitest 통과; 수동 시나리오 체크리스트 전건(복사 → 외부 붙여넣기 → '요약 및 정리' 저장 → 재로딩 후 표시 포함) |
...
| 8 | 분석 슬롯(인터페이스·레지스트리·테이블·자동 분석 API 501·화면 자리) | P0~P3 |
| 9 | 방법론 도구(템플릿·에이전트·스킬·verify_docs·check_env) | P0 |
| 10 | **스크립트 원클릭 복사** + **'요약 및 정리' 수동 입력·편집·저장**(`analyses` `manual`, v2) | P3 |

### Out-of-Scope (v1)

| 항목 | 이유 | 재검토 |
|---|---|---|
| **자동** 요약·핵심 도출·챕터 분석(LLM·추출식) 구현 | 사용자 결정 — 추출 완성·배포 후. 수동 입력·저장은 In-Scope 10 | P5 |
| 모바일 앱 | 배포(P4) 이후 | P6 |
