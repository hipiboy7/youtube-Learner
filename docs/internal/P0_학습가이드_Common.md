...
READY 가 아니면 `[FAIL]` 줄이 무엇이 빠졌는지 말해준다. `[WARN]`(디스크 여유 부족; 모델 캐시가 없으면 하나 더)은 작업을 막지 않는다.
...
| ⑥ | 분석 슬롯 / 수동 요약 흐름 | 자동 요약 기능을 나중에 끼울 자리(v1 은 `NullAnalyzer` 만). 수동 흐름(복사 → 외부 AI → '요약 및 정리' 저장)은 v1 | 6절 |
...
**Q. 자동 요약 기능이 v1 에 없는데 왜 `analysis/` 패키지가 있나?**
A. 나중에 붙일 자리를 지금 만들어 두지 않으면 P5 에서 API·화면·DB 를 다시 설계해야 한다. `Analyzer` Protocol·레지스트리·(P1)`analyses` 테이블·(P1)501 API·(P3)화면 탭이 슬롯이다. 완료 기준은 "P5 에서 `analysis/` 밖은 설정 한 줄만 바뀐다" — `scope-definition.md` 2.4절. 단 **수동 흐름**(스크립트 원클릭 복사 → 외부 AI 챗 → '요약 및 정리'에 붙여 넣어 저장)은 v1(P3)이며, 같은 `analyses` 테이블에 `manual` 출처로 들어간다. 계산이 없으니 `Analyzer` 레지스트리를 거치지 않는다(scope v2, 2026-09-14).
...
A. `main()` 이 subprocess 로만 실행되어 측정되지 않았고, 따옴표 주석 처리 분기가 테스트되지 않았다. 인프로세스 테스트 5개를 추가했다(97%) — 실측은 테스트결과서 3절.
...
**Q. 테스트가 228건인데 정상 케이스만 많은 건 아닌가?**
